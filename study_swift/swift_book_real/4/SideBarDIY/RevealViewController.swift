...
class RevealViewController: UIViewController {
    ...
    func setupView() {
        ...
        if let vc = self.storyboard?.instantiateViewController(withIdentifier: "sw_front") as? UINavigationController {
            ...
            // _프론트 컨트롤러에 부모 뷰 컨트롤러가 바뀌었음을 알려준다. 인터페이스 빌더를 사용하지 않으면 이런식으로 3단계를 거쳐 부모,자식 관계를 설정해줘야함
    ...
    func openSideBar(_ complete: ( () -> Void)? ) {
        ...
        UIView.animate(withDuration: TimeInterval(self.SLIDE_TIME), delay: TimeInterval(0), options: options, animations: {
            self.contentVC?.view.frame = CGRect(x: self.SLIDEBAR_WIDTH, y: 0, width: self.view.frame.width, height: self.view.frame.height)
        }, completion: {
            if $0 == true {
