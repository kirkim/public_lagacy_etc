...
class MainBeminVC: UIViewController {
    ...
    private func attribute() {
        ...
        self.collectionView.register(BeminStaticView.self, forCellWithReuseIdentifier: "test1")
    ...
    private func bind() {
        viewModel.cellData
            .drive(collectionView.rx.items) { cv, row, data in
                print(cv, row, data)
                let cell = cv.dequeueReusableCell(withReuseIdentifier: "test1", for: IndexPath(row: row, section: 0))
                return cell
            }
