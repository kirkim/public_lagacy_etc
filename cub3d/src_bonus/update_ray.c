...
void	cal_ray(t_god *god, t_dpable_ray *hv, int a, int b)
{
		...
		&& next_touchY >= 0 && next_touchY <= god->map.window_height)
	{
		...
		if (is_wall(god, next_touchX + a, next_touchY + b) || door != FALSE)
		{
			...
			hv->wall_hit_x = next_touchX;
			hv->wall_hit_y = next_touchY;
			break ;
		}
...
void	update_one_ray(t_god *god, double angle, int i)
{
	...
	if (vert.distance < horz.distance)
	{
		...
		god->rray[i].wall_hit_x = vert.wall_hit_x;
		god->rray[i].wall_hit_y = vert.wall_hit_y;
	...
	else
	{
		...
		god->rray[i].wall_hit_x = horz.wall_hit_x;
		god->rray[i].wall_hit_y = horz.wall_hit_y;
